...
#' If competitor scores are not arranged as above, set comp.inverted == F and this function will invert competitor scores for you
#' 
#' Comparisons are made using: single-cell Spearman correlation, AUC, and phenotypic Spearman correlation
#' Single-cell Spearman correlation: correlation between single-cell values of ground truth vs. stemFinder score
#' Phenotypic Spearman correlation: correlation between mean values of ground truth vs. stemFinder score per phenotype-defined cluster
#' 
#' @param adata Seurat object containing metadata with columns "Ground_truth" (numeric), "stemFinder" (numeric), and "competitor" if competitor = T
#' @param competitor logical indicating if a comparison between competitor method and ground truth is desired
#' @param comp.inverted logical indicating if competitor scores are inverted to correspond to pseudotime
#' 
#' @return list containing values from quantitative comparison for the query dataset
#' 
compute_performance_single <- function(adata, competitor = F, comp.inverted = F){
  ...
  prediction.sf = c(mostpotent$stemFinder, leastpotent$stemFinder)
  ...
  if (competitor == T){ 
    ...
    if(comp.inverted == F){
      comp_scores_raw = adata[['competitor']]
      comp_scores = 1 - comp_scores_raw/max(comp_scores_raw)
    } else if(comp.inverted == T){
      comp_scores = adata[['competitor']]}
